...
namespace LogicPuzzleSolver.Utilities
{
    ...
    {
        public static System.Windows.RoutedEventHandler ChooseNumberWindow(int maxNumber)
        {
            return (sender, e) => {
                WindowChooseNumber w = new WindowChooseNumber(maxNumber);
                w.ShowDialog();
                if (w.result != 0)
                {
                    ((System.Windows.Controls.Button)sender).Content = w.result;
                }
                else
                {
